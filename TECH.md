...
## Master/Slave 상품 아키텍처

> 2026-04-16 설계 — ads DB

### 개념

오너클랜 상품을 **마스터**로 관리하고, 스마트스토어·11번가·G마켓 등 각 마켓에 등록하는 상품을 **슬레이브**로 분리하여 사이트별 가격/상품명/옵션을 독립 관리한다.

```
ownerclan_product (마스터)
│  W코드 UNIQUE, orig_*=오너클랜원본, 현재값=내수정값
│  sale_status(품절/단종) 중앙관리 → 전체 슬레이브 전파
│
├── master_option (옵션 조합)
│   └── 블랙_XL, 추가금=0, 재고=500  (행 단위)
│
├─→ slave_product (스마트A, seller_a, 수익률35%, 무료배송)
│   └── slave_option (옵션별 추가금/재고 오버라이드)
│
├─→ slave_product (스마트B, seller_b, 수익률30%, 착불3000)
│
└─→ slave_product (11번가, shop_11, 수익률40%)
```
...
| 테이블 | 컬럼 | 용도 |
|--------|------|------|
| `ownerclan_product` | ~120 | 마스터 상품 (기존, 변경없음) |
| `marketplace_store` | 17 | 판매 사이트/셀러 계정 |
| `master_option` | 12 | 마스터 옵션 조합 (행 단위) |
| `slave_product` | 72 | 마켓별 등록 상품 (56개 오버라이드 + 9 link 그룹) |
| `slave_option` | 8 | 슬레이브 옵션 오버라이드 |

### 9개 link 그룹

슬레이브 필드를 그룹별로 마스터 연동(1) 또는 독립(0) 선택.

| link 플래그 | 기본 | 포함 필드 |
|-------------|------|-----------|
| `link_basic` | 1 | 상품명, 마켓상품명, 등급, 카테고리 |
| `link_price` | **0** | 원가, 소비자가, 마켓가, 배송, 수량제한, 과세 |
| `link_images` | 1 | 이미지 대/중/소 |
| `link_detail` | 1 | 키워드, 헤더, 상세HTML |
| `link_options` | 1 | 옵션1/2, 결합/독립옵션 |
| `link_info` | 1 | 제조사, 브랜드, 모델, 원산지, 속성 |
| `link_compliance` | 1 | 인증, 연령, 반품 |
| `link_notice` | 1 | 공지 6개 필드 |
| `link_market` | 1 | 마켓등록 플래그, 프로모션, 사은품 |

### 전파 규칙

- **sale_status 변경** → 전체 슬레이브 `is_synced=0`
- **원가 변경** → 전체 슬레이브 `is_synced=0` (판매가 재계산 필요)
- **콘텐츠 변경** → 해당 link=1인 슬레이브만 `is_synced=0`
- UI에서 "미적용 N개" 표시 → 사용자 "적용" 클릭 → 업데이트

### 가격 계산

```
기본 판매가 = ownerclan_price × (1 + profit_rate / 100)
옵션 판매가 = (ownerclan_price + option.add_price) × (1 + profit_rate / 100)
```

### 옵션 데이터 (combined_option → master_option)

기존 텍스트 형식:
```
블랙_XL,0,9999        ← 옵션값, 추가금, 재고
블랙_2XL,1000,9999
화이트_XL,0,500
```

→ `master_option` 테이블 행 단위로 파싱 저장.
→ `slave_option`에서 사이트별 추가금/재고/활성 오버라이드.
