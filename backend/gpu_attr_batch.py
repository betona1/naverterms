"""GPU 멀티모달 속성 분류 배치 — GPU(108/111/136) 분산.
등록가능(smartstore_product 실존) 상품의 빈 속성을 상품명+대표이미지+상세로 GPU 분류 → 실등록.

usage: python3 gpu_attr_batch.py <limit> [store_id]
"""
...
wl = WorkerLog('gpu_attr_classify', name='GPU 속성 분류등록', worker_type='attr')
...
def work(seller, sid, host):
    tok = get_token(sid)
    try:
        r = gc.classify_sku(seller, sid, host=host, token=tok, dry_run=False)
        with lock:
            stat['done'] += 1
            if r.get('attrs_set'):
                stat['sku'] += 1; stat['attrs'] += r['attrs_set']
            elif r.get('picked', 0) == 0:
                stat['none'] += 1
            if not r.get('ok'):
                stat['fail'] += 1
    except Exception:
    ...
    with lock:
        if stat['done'] % 10 == 0:
            wl.heartbeat(f"처리 {stat['done']}/{len(targets)} · 등록 {stat['sku']}SKU/{stat['attrs']}속성 "
                         f"· 분류없음 {stat['none']} · 실패 {stat['fail']}")
...
wl.done(f"완료 — 등록 {stat['sku']}SKU / {stat['attrs']}속성 · 분류없음 {stat['none']} · 실패 {stat['fail']} (대상 {len(targets)})", meta=stat)
print('DONE', stat)
